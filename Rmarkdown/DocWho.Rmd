...
Python code used for web crawling and initial data construction can be found at: https://github.com/LaurenceDyer/DocWho-Dataset_construction_EDA_and_ML
...
The data set available is, accordingly, very large, with roughly 240000 lines of dialogue over roughly 330 episodes:
...
setwd("C:/Users/laure/Desktop/python/Doctor Who/home/")
...
```{r Overview, echo=F}
knitr::kable(allEps[1:10,c(1,2,3,5,6,7,8)], format="html") %>%
...
```{r Ratings and runtimes, echo = F, message=FALSE, fig.align='center',fig.height=4,fig.width=15,out.width="100%"}
...
all_ext_p <- all_ext_p[,c(2,10,7,8,9,3,11)]

suppressWarnings(print(ggplot(all_ext_p, aes(" "=Title, x=Index,y=Runtime,fill=Season)) +
...
suppressWarnings(print(ggplot(all_ext_p, aes(" "=Title, x=Index,y=Rating,fill=Season)) +
         geom_line(stat="smooth",aes(group=""),colour="black", se = F, alpha = 0.65, size = 1.5) +
         geom_point(size = 3, shape = 21, colour = "grey20") + 
         theme_minimal() +
...
suppressWarnings(print(ggplot(all_ext_p, aes(" "=Title, x=Index,y=Views,fill=Season)) +
         geom_line(stat = "smooth", aes(group=""), colour="black", se = F, alpha = 0.65, size = 1.5) +
         geom_point(size = 3, shape = 21, colour = "grey20") + 
         theme_minimal() +
         theme(legend.position = "None") +
...
writerCols <- writerDat_f[!duplicated(writerDat_f$Writer),]
n <- length(writerCols$Writer)

writerCols$Col <- hcl.colors(n, "Spectral")

col <- as.character(writerCols$Col)
names(col) <- as.character(writerCols$Writer)
col <- col[!duplicated(col)]

writerDat_f <- merge(writerDat_f,writerCols[,c(1,6)],by="Writer")

g1 <- ggplot(writerDat_f, aes(x=Writer,y=Rating, fill=Col)) + 
          geom_boxplot(aes(group=Writer,colour=Col)) +
...
g2 <- ggplot(writerDat_f, aes(x=Writer,y=Views, fill=Col)) + 
          geom_boxplot(aes(group=Writer,colour=Col)) +
          geom_point(alpha=0.5, position = position_jitter(width = 0.15)) +
          scale_colour_manual(values = col) +
          theme_minimal() +
...
n <- length(char_vals$cat)
char_vals$Col<- hcl.colors(n, "Spectral")

col <- as.character(char_vals$Col)
names(col) <- as.character(char_vals$Character)

suppressWarnings(ggplot(head(char_vals,50), aes(area=pct,fill=Col, label = cat))) +
  geom_treemap() +
  scale_color_manual(values=col) +
  geom_treemap_text(colour = "white",
                    place = "centre",
                    size = 15) + ggtitle("Who has the most lines?") + theme(legend.position="None")
...
suppressWarnings(ggplot(head(char_vals2,50), aes(area=pct,fill=Col, label = cat))) +
  geom_treemap() +
  scale_color_manual(values=col) +
  geom_treemap_text(colour = "white",
                    place = "centre",
                    size = 15) + ggtitle("...Other than the doctor?") + theme(legend.position="None")
```
</div>
...
colnames(char_vals) <- c("Lines","Character","Col")
...
col <- as.character(chars_plot$Col)
names(col) <- as.character(chars_plot$Character)

g1 <- ggplot(chars_plot, aes(x=Character,y=LinesPerEpisode,fill=Col)) + 
...
g2 <- ggplot(chars_plot, aes(x=Character,y=LinesPerEpisode,fill=Col)) + 
        geom_bar(stat="identity") +
        scale_color_manual(values=col) +
        theme_minimal() +
                theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 7), panel.grid.minor = element_blank(),
...
g3 <- ggplot(chars_plot, aes(x=Character,y=WordsPerLine,fill=Col)) + 
        geom_bar(stat="identity") +
        scale_color_manual(values=col) +
        theme_minimal() +
                theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 7), panel.grid.minor = element_blank(),
...
n <- length(docs_norm$Character)
docs_norm$Cols <- hcl.colors(n, "Spectral")

col <- as.character(docs_norm$Col)
names(col) <- as.character(docs_norm$Character)

grid.arrange(ggplot(docs_norm, aes(x=Char,y=Lines,group="")) + 
                geom_point(aes(fill=Char,colour=Cols),size=2.5) + geom_smooth(se=F) + 
             ...
             ggplot(docs_norm, aes(x=Char,y=Series_Episode,group="")) + geom_smooth(se=F) + 
                geom_point(aes(fill=Char,colour=Cols),size=2.5) + ggtitle("Episodes") +
                scale_fill_manual(values=col) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 7), panel.grid.minor = element_blank(),
                      panel.background = element_blank(),legend.position = "None"),
             ...
             ggplot(docs_norm, aes(x=Char,y=LinesPerEpisode,group="")) + geom_smooth(se=F) + 
                geom_point(aes(fill=Char,colour=Cols),size=2.5) + ggtitle("Lines per Episode") +
                scale_fill_manual(values=col) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 7), panel.grid.minor = element_blank(),
                      panel.background = element_blank(),legend.position = "None"),
             ...
             ggplot(docs_norm, aes(x=Char,y=WordsPerLine,group="")) + geom_smooth(se=F) + 
                geom_point(aes(fill=Char,colour=Cols),size=2.5) + ggtitle("Words per Line") +
                scale_fill_manual(values=col) +
                theme_bw() +
                theme(axis.text.x = element_text(angle = 50, hjust = 1, size = 7), panel.grid.minor = element_blank(),
                      panel.background = element_blank(),legend.position = "None"),
...
doc_col_t <- docs_norm[,c(7,9)]
colnames(doc_col_t) <- c("DoctorWho","Cols")
popDoc <- merge(popDoc,doc_col_t,by="DoctorWho")

col <- popDoc[!duplicated(popDoc$DoctorWho),]$Cols
names(col) <- popDoc[!duplicated(popDoc$DoctorWho),]$Cols


suppressWarnings(print(ggplot(popDoc, aes(x=DoctorWho,y=Rating,fill=DoctorWho)) +
...
allEps_charR_lines <- merge(allEps_charR_lines,chars_norm[,c(1,4)],by="Character")

col <- allEps_charR_lines[!duplicated(allEps_charR_lines$Character),]$Col
names(col) <- allEps_charR_lines[!duplicated(allEps_charR_lines$Character),]$Col

col

suppressWarnings(ggplot(allEps_charR_lines, aes(x=Script,y=Rating)) + geom_point(aes(colour=Col),alpha = 0.5) +
...
knitr::include_graphics("./images/Doc_Who_Network.png")
```
...
Perhaps there are some themes that run through particular episodes that make them very popular. We might be able to find these by looking at the top thousand words in each episode and assigning them that rating's episode. Then, we average each word out over each of each of its appearances to find a combined total rating for each word.

Doing this we can arrive at around 15,000 word stems that we can assign a rating to. A lot of these words only appear once, so we'll try to limit our analysis to only words with a frequency greater than 100.

We'll also do our analysis once with names, and once without - Perhaps we will find some characters that are more popular when spoken about than when they are speaking themselves. To get a good idea of word frequency, we'll also add a size parameter, with larger sizes indicating more frequent words.

```{r TFID and Rating, echo = F, fig.align='center', fig.width=12, fig.height=6, warning=FALSE, message=FALSE}
...
top_feats_title <- topfeatures(all_freq_mat, 1000, group = all_freq_mat@docvars$Title)

top_feats_title_tf <- as.data.frame(sapply(top_feats_title, 
                                    function(x) names(x)))

top_feats_title_tf <- as.data.frame(t(top_feats_title_tf))
top_feats_title_tf$Title <- rownames(top_feats_title_tf)
top_feats_title_tf <- merge(top_feats_title_tf, allEps[,c(11,15)][!duplicated(allEps[,c(11,15)]$Title),],
                            by="Title", all.y=F)

top_feats_title_tf_rating <- melt(top_feats_title_tf[,-1], id.vars = "Rating")
top_feats_title_tf_rating <-top_feats_title_tf_rating[,-2]
top_feats_title_tf_rating <- top_feats_title_tf_rating[!is.na(top_feats_title_tf_rating$Rating),]

top_feats_title_tf_agg <- aggregate(Rating~value,top_feats_title_tf_rating,mean)

wordFreq <- as.data.frame(featfreq(all_freq_mat))

wordFreq$value <- rownames(wordFreq)

word_frame <- merge(top_feats_title_tf_agg,wordFreq,by="value")
word_frame_f <- word_frame[word_frame$`featfreq(all_freq_mat)` > 100,]
colnames(word_frame_f) <- c("Word","Rating","Freq")

word_frame_f$Word <- factor(word_frame_f$Word, levels = rev(unique(word_frame_f[order(word_frame_f$Rating,decreasing = T),]$Word)))
word_frame_f <- word_frame_f[order(word_frame_f$Rating, decreasing = T),]

g1 <- ggplot(head(word_frame_f,30), aes(y=Word,x=Rating,size=Freq)) +
          geom_point() + ggtitle("With names...") +
  theme_minimal() + theme(legend.position = "None") + xlab("Rating Score")

word_frame_f <- word_frame_f[!word_frame_f$Word %in% names,]
word_frame_f <- word_frame_f[-c(2,3,4),]

word_frame_f <- word_frame_f[order(word_frame_f$Rating, decreasing = T),]

g2 <- ggplot(head(word_frame_f,30), aes(y=Word,x=Rating,size=Freq)) +
          geom_point() + ggtitle("...and without.") +
  theme_minimal() + theme(legend.position = "None") + xlab("Rating Score")
...
```
Well, interesting! Christmas, merry and stories are very popular, probably owing to their use in christmas-themed one-offs which are always enjoyed by fans. We also see that "Davros" stands out as a very popular character to reference, along with dalek, daleks and exterminate. Surprisingly, monsters appear to come up very frequently here, with words such as yeti, mummy, angels (A stone statue that moves when you blink) and ghosts. Monster of the week is clearly a popular trope!
...
g1 <- ggplot(char2_epSentiment, aes(x=Doc_Sentiment,y=Oth_Sentiment)) + geom_point(stroke = 1, shape=21, size = 2, aes(fill=as.factor(Era))) + 
        geom_smooth(colour="red",method="lm", se = F) + theme_minimal() + theme(legend.position = "none") + ggtitle("pval=0.226")
g2 <- ggplot(char2_epSentiment, aes(x=Doc_Sentiment,y=Rating)) + geom_point(stroke = 1, shape=21, size = 2, aes(fill=as.factor(Era))) + 
              geom_smooth(colour="red",method="lm", se = F) + theme_minimal() + theme(legend.position = "none") + ggtitle("pval=0.006")
g3 <- ggplot(char2_epSentiment, aes(x=Doc_Sentiment,y=`Viewers(millions)`)) + geom_point(stroke = 1, shape=21, size = 2, aes(fill=as.factor(Era))) + 
              geom_smooth(colour="red",method="lm", se = F) + theme_minimal() + theme(legend.position = "none") + ggtitle("pval=0.630")
...
The natural language of a TV script provides a fairly complicated feature space for us to study. In this section we'll attempt to construct and hopefully optimise a few machine learning models.
...
```{r Naive Bayes output2, out.extra='style="max-width:none; width:60vw; margin-left:calc(50% - 30vw);"', fig.width = 150, fig.height = 50,echo=FALSE}

knitr::include_graphics("./images/naiveBayes.png")
...
```{r Naive Bayes output3, out.extra='style="max-width:none; width:40vw; margin-left:calc(50% - 20vw);"', fig.width = 150, fig.height = 50,echo=FALSE}

knitr::include_graphics("./images/naive_bayes2.png")
...
```{r Naive Bayes output4, out.extra='style="max-width:none; width:40vw; margin-left:calc(50% - 20vw);"', fig.width = 150, fig.height = 50,echo=FALSE}

knitr::include_graphics("./images/naive_bayes_era.png")
...
We’ll attempt to create our regression model using the python sklearn toolkit, and we’ll test out the Lasso regularization model.
...
```{r LASSO res, echo = FALSE, out.extra='style="max-width:none; width:60vw; margin-left:calc(50% - 30vw);"', fig.width = 105, fig.height = 35,echo=FALSE}
...
```{r LSTM output2, out.extra='style="max-width:none; width:60vw; margin-left:calc(50% - 30vw);"', fig.width = 150, fig.height = 50,echo=FALSE}

knitr::include_graphics("./images/text_generation_One.png")
...
```{r LSTM output3, out.extra='style="max-width:none; width:60vw; margin-left:calc(50% - 30vw);"', fig.width = 150, fig.height = 50,echo=FALSE}

knitr::include_graphics("./images/LSTM_Dalek.png")
...
Beyond that, we do seem to have some great sentences! They are almost readable and certainly sound Dalek-y! Exterminate, exterminate, exterminate, exterminate, exterminate, exterminate, exterminate...
